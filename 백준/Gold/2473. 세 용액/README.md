...
### 해결 방안
 ...
 <p>첫 해결 방안으로 문제를 보자마자 permutation을 생각하고 코드를 짜 풀어보니 시간초과가 나왔다. -> 모든 경우를 탐색하는 것이 아니다</p>
 <p>무조건 세 용액을 고르면서 최소 절대값을 찾는것이므로 하나를 무조건 선택한다는 가정하에는 2개의 용액만 고르면 된다. 여기서 2개의 용액을 정렬한 배열 기준 첫번째 index와 마지막 index로 시작하여 세 용액의 합을 비교해가면서 최소값을 찾아나선다. => 문제 풀고 나서 생각든게 next_Permutation으로 풀면 안풀리나??</p>
