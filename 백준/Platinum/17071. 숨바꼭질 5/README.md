...
### 해결 방안
<p> 수빈이는 -1, +1, *2 로 이동할 수 있기 때문에 한번 방문한 점은 2초 마다 다시 반복할 수 있다. 이러한 경우를 대비해 각 지점별 도착시간을 저장한다. 하지만 수빈이가 짝수 시간에 방문하고 동생이 같은 지점에 홀수 시간에 방문한다면 수빈이는 동생을 만나지 못한다. 따라서 수빈이가 각 지점에 도착했을 당시 시간이 짝수 인지, 홀수 인지를 구분하기 위해 배열을 2차원 배열로 사용했다.</p>
